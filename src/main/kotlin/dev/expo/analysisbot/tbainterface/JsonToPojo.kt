...
object JsonToPojo {
    ...
    fun populate(data: JsonObject, target: Any) {
        for (field in target.javaClass.fields) {
            ...
            if (field.type.isAnnotationPresent(JsonInclude::class.java)) {
                ...
                when (field.type) {
                    Class.forName("java.lang.Integer") -> {
                        val num = data.get(jsonFieldName).asInt
                        field.set(target, Integer.valueOf(num))
                    }
                        ...
                        for (d in ls.asJsonArray) {
                            if (d.isJsonObject) {
                                val listTypeClass = Class.forName(
                                    (field.genericType as ParameterizedType).actualTypeArguments[0].toString()
                                        .substring(6)
                                )
    ...
    init {
        config = object : DefaultGenerationConfig() {
            override fun isGenerateBuilders(): Boolean { return false }
            override fun isUsePrimitives(): Boolean { return false }
            override fun isIncludeGetters(): Boolean { return false }
            override fun getAnnotationStyle(): AnnotationStyle { return AnnotationStyle.NONE }
            override fun getSourceType(): SourceType { return SourceType.JSON }
            override fun isIncludeSetters(): Boolean { return false }
            override fun isIncludeToString(): Boolean { return false }
            override fun isIncludeAdditionalProperties(): Boolean { return true }
            override fun isIncludeHashcodeAndEquals(): Boolean { return false }
            override fun isIncludeConstructorPropertiesAnnotation(): Boolean { return false }
            override fun isIncludeJsr305Annotations(): Boolean { return false }
            override fun isIncludeGeneratedAnnotation(): Boolean { return true }
            override fun isIncludeJsr303Annotations(): Boolean { return false }
            override fun isInitializeCollections(): Boolean { return false }
        }
